            ...
            <div class="container bg-light-red p-4">
                <div class="row justify-content-around">
                    ...
                    <div class="col-3 " style="overflow: auto; height: 90vh;">
                        <div class="pokemon-card px-1 d-flex justify-content-between bg-white align-items-center cursor-pointer" :class="activePokemon(item.game_indices[16].game_index)" v-for="item in store.pokemons" @click="changePokemon(item.game_indices[16].game_index), activePokemon(item.game_indices[16].game_index)">
                            ...
                            <p class="fs-6 m-0 pe-2">Nr. {{ formatDexEntry(item.game_indices[16].game_index) }}</p>
                        </div>
                    </div>
                </div>
            </div>
...
ChartJS.register(
    ...
    export default {
        ...
        methods: {
            ...
            changePokemon(entry){
                const selectedPokemon = store.pokemons.find(pokemon => pokemon.game_indices && pokemon.game_indices[16].game_index === entry);
                if (selectedPokemon) {
            ...
            activePokemon(entry){
                if(store.pokemons[this.activeIndex].game_indices[16].game_index == entry){
                    return 'bg-gray';
                }
            },
            formatNumber(number){
        ...
        mounted(){
            this.getPokemons().then(() => {
                if (store.pokemons.length > 0) {
                    ...
                    this.changePokemon(store.pokemons[0].game_indices[16].game_index);
                }
            });
        }
    }
